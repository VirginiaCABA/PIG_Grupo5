...
from .forms import ContactoForm, PedidoForm, ClienteForm, PaqueteForm, DomicilioForm
...
def contacto(request):
    ...
    if request.method == 'GET':
        ...
        if formulario.is_valid():
            ...
            if Postulante.objects.filter(email=email).exists():
                respuesta = HttpResponseBadRequest(f"El Email {email} ya esta registrado.")
            else:
                #guardar los datos en la BD
                nuevo_postulante = Postulante(first_name=first_name, last_name=last_name, dni=dni,
                                        email=email, mensaje=mensaje, curriculum=curriculum)
                nuevo_postulante.save()
                #formatear mensaje de respuesta
                mensaje_html = f'Mensaje de contacto de {first_name} {last_name} (DNI: {dni}):\n {mensaje}'
...
class DomicilioCreateView(CreateView):
    ...
    template_name = 'core/pages/registrar_domicilio.html'
    form_class = DomicilioForm

    def post(self, request, *args, **kwargs):
        form_domicilio = self.form_class(request.POST)
        if form_domicilio.is_valid():
            domicilio = form_domicilio.save(commit=False)
            form_domicilio.save()
        return HttpResponseRedirect('/registrar_cliente/' + str(domicilio._get_pk_val()))
...
class ClienteCreateView(CreateView):
    ...
    def get_initial(self):
        ...
        if self.request.method == "GET":
            id_domicilio = self.kwargs['domicilio']
            domicilio = Domicilio.objects.filter(id=id_domicilio).first()
            initial['domicilio'] = domicilio
        return initial
